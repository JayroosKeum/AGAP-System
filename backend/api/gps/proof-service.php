...
if ($_SERVER['REQUEST_METHOD'] === 'GET') { $caseId = filter_input(INPUT_GET, 'case_id', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]); $mode = $_GET['mode'] ?? ''; $result = $caseId ? ($mode === 'documents' ? $controller->documents((int) $caseId) : $controller->proofs((int) $caseId)) : $controller->cases(); http_response_code($result['success'] ? 200 : 422); echo json_encode($result); exit; }
