...
if ($method === 'GET') {
    ...
    respond(['success' => true, 'data' => $controller->getAttachments((int) $complaintId)]);
}

if ($method === 'POST') {
    $complaintId = filter_var($_POST['complaint_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if (!$complaintId || !isset($_FILES['attachment'])) {
        respond(['success' => false, 'message' => 'A valid complaint and attachment are required.'], 422);
    }
    $result = $controller->addAttachment((int) $complaintId, $_FILES['attachment']);
    respond($result, $result['success'] ? 201 : 422);
}

if ($method === 'DELETE') {
    parse_str(file_get_contents('php://input'), $input);
    $attachmentId = filter_var($input['attachment_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if (!$attachmentId) {
        respond(['success' => false, 'message' => 'A valid attachment ID is required.'], 422);
    }
    $result = $controller->deleteAttachment((int) $attachmentId);
